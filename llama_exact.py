...
def main(cfg):
    ...
    log_file_path = cfg.model_path+f'/exact_{cfg.update_}.log'
    ...
    for i, j in enumerate(forget_loader):
        ...
        #log1.info('ground_truth:')
        #log1.info(j['answer'][0])
        #log1.info('answer_init:')
        #log1.info(ans)
        #ans=generated_prompt[0][:].split('\n')[0]
        #time.sleep(2)
        if ans == j['answer'][0]:
        ...
        for k in range(gt_length):
            ...
            #log1.info(f'{k+1} words:')
            #log1.info(' '.join(j['answer'][0].split(" ")[:k + 1])+ans)
