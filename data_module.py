...
class TextForgetDatasetQA(Dataset):
    def __init__(self, data_path, tokenizer, model_family,  max_length=512, split = "forget10", loss_type="idk"):
        ...
        self.retain_data1=self.retain_data.train_test_split(test_size=100)['train']
        self.retain_data2 = self.retain_data.train_test_split(test_size=100)['test']
        self.model_configs = get_model_identifiers_from_yaml(model_family)
        self.loss_type = loss_type

        if self.loss_type == "idk":
            self.split1, self.split2, self.split3 = "idk", "retain", "retain_eval"
            self.idontknowfile = "data/idontknow.jsonl"
            self.idk = open(self.idontknowfile, "r").readlines()
        else:
            self.split1, self.split2, self.split3 = "forget", "retain", "retain_eval"
    ...
    def __getitem__(self, idx):
        ...
        for data_type in [self.split1, self.split2]:
            ...
            if data_type == "retain":
                data = self.retain_data1
                idx = (idx + torch.randint(0, len(self.retain_data1), (1,)).item()) % len(self.retain_data1)
            else:
...
class TextForgetDatasetQA2(Dataset):
    def __init__(self, data_path, tokenizer, model_family,  max_length=512, split = "forget10", loss_type="idk"):
        ...
        retain_split = "retain" + str(100 - int(split.replace("forget", ""))).zfill(2)
        self.retain_data =datasets.load_dataset(data_path, retain_split)["train"]
        data_f=pd.DataFrame(self.retain_data).iloc[400:].reset_index(drop=True)
        self.retain_data1=datasets.Dataset.from_pandas(data_f)
        self.model_configs = get_model_identifiers_from_yaml(model_family)
        self.loss_type = loss_type
...
class TextForgetDatasetDPOQA2(Dataset):
    def __init__(self, data_path, tokenizer, model_family, max_length=512, split = "forget10", ):
        super(TextForgetDatasetDPOQA2, self).__init__()
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.forget_data = datasets.load_dataset(data_path, split)["train"]
        self.idontknowfile = "data/idontknow.jsonl"
...
def get_batch_loss_ocr(output, labels, delta=2.,gamma=0.05):
    shifted_labels = labels[..., 1:].contiguous()
    output = output[..., :-1, :].contiguous()
    ocr_loss=0.0
    ocr_count=0
